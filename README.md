...
Fresh verification passed: **36 Python tests, four native renderer builds, and all six full-geometry renders**. See [the committed proof gallery and receipts](docs/recovery-proof/README.md) and [the complete successful run](https://github.com/cybrdelic/cybr-scenes/actions/runs/35670895190). Pushes run tests/builds; use **Run workflow** to opt into the expensive six-scene render matrix.

For a new composition, run `python tools/create_postcard.py` from `environments/`, or select the **Create Amber Passage postcard** workflow. It reuses the original sandstone geometry with a tighter field of view, 16:10 framing and 5200K finishing white balance; the original preset remains unchanged.
...
**[Download current source and assets](https://github.com/cybrdelic/cybr-scenes/archive/refs/heads/master.zip)** · [Full-resolution render](scenes/observatory-iv/renders/Observatory_IV.png?raw=true) · [Unfiltered image](scenes/observatory-iv/renders/Observatory_IV_unfiltered.png?raw=true) · [Scene documentation](scenes/observatory-iv/README.md)
...
`requirements.txt` selects the exact versions in `requirements-tested.txt`. This avoids the OpenCV 5.0.0 wheel encountered during publication, which had no EXR writer. The tested dependency set uses OpenCV 4.13.0.92; full finishing and raw-output validation require EXR support.
...
The [original delivery ZIP](downloads/CYBR_Observatory_IV_Source.zip?raw=true) is 14,922,796 bytes with SHA-256 `ebf85d46b54a510915cf48e15f14944cac33e81be3f7a9e9618f29a38f549a0a`. It preserves the original package layout and remains unchanged for provenance. The repository places that scene under `scenes/observatory-iv/` and fixes its dependency selection. Use the current-source download above for the publication fixes. When using the original delivery ZIP instead, install `requirements-tested.txt` rather than its original unbounded `requirements.txt`.
