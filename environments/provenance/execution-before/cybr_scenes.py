...
def exclusive(path:Path):
    """Protect outputs and compilation from overlapping invocations."""
    path.parent.mkdir(parents=True,exist_ok=True)
    fd=None
    try:
        fd=os.open(path,os.O_CREAT|os.O_EXCL|os.O_WRONLY,0o600)
        os.write(fd,f'{os.getpid()} {now()}\n'.encode());os.close(fd);fd=-1
        yield
    except FileExistsError:
        raise RuntimeError(f'Locked: {path}. Verify no process is active before removing a stale lock.')
    finally:
        if fd is not None:
            if fd>=0:os.close(fd)
...
def environment(threads:int):
    return {**os.environ,'OPENBLAS_NUM_THREADS':'1','OMP_NUM_THREADS':str(threads),
       'NUMBA_NUM_THREADS':str(max(4,threads)),'PYTHONUNBUFFERED':'1',
       'CYBR_SURFACE_ATLAS':str(ROOT/'assets/mineral_detail.cdt')}


def run(command,cwd:Path,log:Path,threads:int=4,timeout:int=14400):
    log.parent.mkdir(parents=True,exist_ok=True)
    command=list(map(str,command));start=time.monotonic()
    record={'command':command,'cwd':relative(cwd),'started_at':now(),
       'environment':{k:v for k,v in environment(threads).items() if k in ['OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','NUMBA_NUM_THREADS','CYBR_SURFACE_ATLAS']}}
    atomic_json(log.with_suffix('.command.json'),record)
    print(f'[{now()}] {log.stem}: {" ".join(command)}',flush=True)
    try:
        resource_path=log.with_suffix('.resources.txt')
        executed=['/usr/bin/time','-v','-o',str(resource_path),*command] if Path('/usr/bin/time').exists() else command
        record['executed_command']=executed
        with log.open('w') as f:
            process=subprocess.Popen(executed,cwd=cwd,stdout=f,stderr=subprocess.STDOUT,
                env=environment(threads),start_new_session=True)
            try:
                code=process.wait(timeout=timeout)
            except subprocess.TimeoutExpired:
                # Stop the entire time/native child group, not only the timing wrapper.
                os.killpg(process.pid,signal.SIGTERM)
                try:process.wait(timeout=5)
                except subprocess.TimeoutExpired:
                    os.killpg(process.pid,signal.SIGKILL);process.wait()
                raise
            completed=subprocess.CompletedProcess(executed,code)
        record.update(returncode=completed.returncode,seconds=time.monotonic()-start,finished_at=now(),log_sha256=sha(log))
        atomic_json(log.with_suffix('.command.json'),record)
        if completed.returncode:
            tail='\n'.join(log.read_text(errors='replace').splitlines()[-18:])
            raise RuntimeError(f'Command failed ({completed.returncode}), {log}:\n{tail}')
    except subprocess.TimeoutExpired:
        record.update(timed_out=True,seconds=time.monotonic()-start);atomic_json(log.with_suffix('.command.json'),record);raise
    return record
    ...
    try:
        if args.command=='list':print(json.dumps(REGISTRY,indent=2))
            ...
            for scene in selected:
                ...
                outcomes.append({'scene':scene['id'],'result':str(result) if isinstance(result,Path) else result})
