...
def source_selected(p):
 ...
 if p.suffix.lower() in FONTS or any(x in parts for x in ['__pycache__','.git','.part-spool']):return False
 if p.suffix in ['.pyc','.nbc','.nbi','.partial','.o','.lock']:return False
 if parts[0] in ['geometry','geometry-r2','build']:return False
...
def archive(output,files):
 ...
 with zipfile.ZipFile(output,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=6,allowZip64=True) as z:
  for p in sorted(files):
   r=p.relative_to(ROOT);z.write(p,Path(ROOT.name)/r)
