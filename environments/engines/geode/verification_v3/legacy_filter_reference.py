...
def atrous(rgb:np.ndarray, guide:np.ndarray, variance:np.ndarray,
    ...
    for iteration in range(iterations):
        ...
        arrays=[data,normals,depth,material,variance,loglum,albedo]
        ...
        for ky in range(-2,3):
            for kx in range(-2,3):
                ...
                c,n,d,m,var,ll,a=(v[sl] for v in padded)
                if kind=='volume':
                ...
                sigma=.055+2.3*np.sqrt(np.maximum(variance+var,0))*4/(1+luminance*4)
        ...
        variance=np.maximum(next_variance/(den*den),variance*.15)
...
def main()->None:
    ...
    meta['finishing']={'method':'first-interface-aware base / water reflection / atmosphere wavelet filtering; non-neural','iterations':{'base':3,'reflection':4,'volume':5},'raw_modified':False,'outlier_control':{'enabled':not args.no_outlier_control,'affected_pixels':controlled,'biased_viewing_regularization':True,'peak_ratio':8,'absolute_margin':.35},'display_transform':'ACES fit / exact sRGB','exposure':exposure,'image_generation':False,'neural_denoiser':False}
