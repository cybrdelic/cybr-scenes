...
window.SandstoneQuality=class SandstoneQuality {
 constructor({renderer,scene,skyScene,camera,uniforms,skyUniforms,detail,bounds}){
  ...
  this.presentUniforms={uDither:{value:1},uImage:{value:null},uWhite:uniforms.uWhite,uExposure:uniforms.uExposure,uMode:uniforms.uMode};
  this.presentMaterial=new THREE.RawShaderMaterial({glslVersion:THREE.GLSL3,vertexShader:vertex,
   fragmentShader:`precision highp float;precision highp int;precision highp sampler2D;in vec2 vUV;uniform sampler2D uImage;uniform vec3 uWhite;uniform float uExposure;uniform int uMode;uniform int uDither;out vec4 outColor;
