...
constexpr float PI=3.14159265358979323846f, EPS=.000008f, INF=1.e30f;
struct V {
 float x=0,y=0,z=0;
...
// Per-pixel digital shift over linearly scrambled 1024-dimensional Sobol nets.
// The PRNG remains available for diagnostics and high-dimensional fallbacks.
inline uint32_t mix32(uint32_t x){x^=x>>16;x*=0x7feb352du;x^=x>>15;x*=0x846ca68bu;x^=x>>16;return x;}
struct RNG{
 uint64_t state; uint32_t sampleIndex=0,pixelKey=0,dimension=0; bool qmc=false;
 ...
 void beginSample(uint32_t index,uint32_t key){sampleIndex=index;pixelKey=key;dimension=0;qmc=true;state=(uint64_t(mix32(index^key))<<32)|mix32(index+key+1);state|=1;}
 float uniform(){
  if(qmc&&dimension<1024){uint32_t dim=dimension++,i=sampleIndex,v=mix32(pixelKey^mix32(dim+1837));
   while(i){int b=__builtin_ctz(i);v^=SOBOL_DIRECTIONS[dim][b];i&=i-1;}
   return ((v>>8)+.5f)*(1.f/16777216.f);
  }return ((next()>>8)+.5f)*(1.f/16777216.f);
 }
};
...
bool meshHitReference(const Ray&r,Hit &h,bool any=false){
...
// Validated-child traversal avoids testing the same node bounds twice.
// Triangles and per-corner attributes are reordered into contiguous BVH leaves.
bool meshHit(const Ray&r,Hit &h,bool any=false){
 int stack[128],sp=0;bool found=false;if(nodes.empty()||!nodes[0].b.hit(r,h.t))return false;stack[sp++]=0;
 while(sp){const auto &nd=nodes[stack[--sp]];
  if(nd.count){for(int i=nd.start;i<nd.start+nd.count;i++){int id=order[i];float t,u,v;
   if(tris[id].hit(r,h.t,t,u,v)){h.t=t;h.u=u;h.v=v;h.tri=id;h.floor=false;h.light=-1;found=true;if(any)return true;}}}
  else{float a,b;bool ha=nodes[nd.left].b.hit(r,h.t,&a),hb=nodes[nd.right].b.hit(r,h.t,&b);
   if(ha&&hb){if(a<b){stack[sp++]=nd.right;stack[sp++]=nd.left;}else{stack[sp++]=nd.left;stack[sp++]=nd.right;}}
   else if(ha)stack[sp++]=nd.left;else if(hb)stack[sp++]=nd.right;
  }
 }return found;
